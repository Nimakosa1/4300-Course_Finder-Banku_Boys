...
def calculate_average_ratings(reviews):
    if not reviews:
        return {'avgDifficulty': 0, 'avgWorkload': 0, 'avgOverall': 0}
    difficulty_sum = difficulty_count = workload_sum = workload_count = overall_sum = overall_count = 0
    for review in reviews:
        ...
        if 'overall' in review and review['overall'] != '-':
            try:
                ...
                pass
    return {
        'avgDifficulty': difficulty_sum / difficulty_count if difficulty_count else 0,
        'avgWorkload': workload_sum / workload_count if workload_count else 0,
        'avgOverall': overall_sum / overall_count if overall_count else 0
    }
...
def simple_search(query, courses):
    ...
    for idx, course in enumerate(courses):
        score = 0
        if 'course_code' in course and query in course['course_code'].lower(): score += 10
        if 'course title' in course and query in course['course title'].lower(): score += 5
        if 'description' in course and query in course['description'].lower(): score += 3
        if score > 0: results.append((score, idx))
...
def api_search():
    try:
        ...
        search_results = []
        if search_function and inv_idx and idf is not None and doc_norms and len(doc_norms) > 0:
            try:
        ...
        for score, idx in top_results:
            if idx < len(courses_list): result.append(courses_list[idx])
        formatted_results = []
...
def api_get_course(course_id):
    try:
        ...
        course_info = next((c for c in courses_list if c.get("course_code") == original_course_id), None)
        if not course_info:
            return jsonify({"error": "Course not found"}), 404
        course_info_clean = course_info.copy()
        if "description_tokens" in course_info_clean:
...
def api_get_courses():
    ...
    for course in courses_list:
        ...
        if "description_tokens" in course_copy:
            del course_copy["description_tokens"]
        final_courses.append(course_copy)
    return jsonify({"courses": final_courses, "keywords": []})
